...
connection.on("ReceiveMessage", function (message, username, photo) {
    if (!isShowing) {
        showChat(username);
        ...
        document.getElementById("card-body").innerHTML += msg;
    }

});

connection.on("GetOnlines", function (users, username) {
    ...
    for (var key in users) {
        ...
        if (key != ThisUser) {
            var online = "<div class=\"user\" id=\"" + username + "\">" +
                "<div class=\"profile\">" +
                //"<img src=\"/img/" + users[key]["profilePhoto"] + "\">" +
                //"</div>" +
                "<h4 onclick=\"showChat(" + "'" + key + "'" + ")\">" +
                users[key]["name"] +
                "</h4 > " +
                "</div>";
...
connection.on("SetUserName", function (username) {
    ThisUser = username;
});

//connection.start().then(function () {
//    //document.getElementById("sendButton").disabled = false;
//    connection.invoke("ConnectedUser").catch(function (err) {
...
function Send() {
    ...
    connection.invoke("SendMessage", message, username).catch(function (err) {
        return console.error(err.toString());
    });
    event.preventDefault();
}

connection.on("ConnectMessage", function (username, fullname, photo) {
    //var li = document.createElement("li");
    //var li = "<li onclick=\"showChat(" + "'" + username + "'" + ")\">" + surname + "</li>"

    var online = "<div class=\"user\" id=\"" + username + "\">" +
        "<div class=\"profile\">" +
        "<img src=\"/img/" + photo + "\">" +
        "</div>" +
        "<h4 onclick=\"showChat(" + "'" + username + "'" + ")\">" +
        fullname +
        "</h4 > " +
        "</div>";
    //li.textContent = user;
...
connection.on("ReqAlert", function (message) {
    document.getElementById("notification").classList.add("has-notification");
    alert(message + " sana arkadaşlık isteği yolladı");
});

//*************BURADAN SONRA CRUD İŞLEMLERİ*******************************//
...
function showChat(username) {
    ...
    $.ajax({
        method: "POST",
        url: "/Facebook/ShowChat",
        data: UserName,
        async: false,
        success: function (response) {
            $("#chat-partial").html(response);
